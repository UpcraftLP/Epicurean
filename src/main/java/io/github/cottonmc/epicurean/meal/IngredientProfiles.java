...
public class IngredientProfiles implements SimpleResourceReloadListener {
	...
	public CompletableFuture<Void> apply(Object data, ResourceManager manager, Profiler profiler, Executor executor) {
		return CompletableFuture.runAsync(() -> {
			...
			DRESSINGS.clear();
			Tag<Item> SPICY = ItemTags.getTagGroup().get(new Identifier(Epicurean.MOD_ID, "spicy"));
			Tag<Item> UMAMI = ItemTags.getTagGroup().get(new Identifier(Epicurean.MOD_ID, "umami"));
			Tag<Item> ACIDIC = ItemTags.getTagGroup().get(new Identifier(Epicurean.MOD_ID, "acidic"));
			Tag<Item> SWEET = ItemTags.getTagGroup().get(new Identifier(Epicurean.MOD_ID, "sweet"));
			Tag<Item> BITTER = ItemTags.getTagGroup().get(new Identifier(Epicurean.MOD_ID, "bitter"));
			Tag<Item> FILLING = ItemTags.getTagGroup().get(new Identifier(Epicurean.MOD_ID, "filling"));
	...
	private void putTag(Tag<Item> tag, FlavorGroup flavor) {
		Tag<Item> DRESSING_TAG = ItemTags.getTagGroup().get(new Identifier(Epicurean.MOD_ID, "dressings"));
